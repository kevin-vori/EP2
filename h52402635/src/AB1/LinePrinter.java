...
public class LinePrinter {
    ...
    private void createLineBuffer(int lineLength, int cellHeight, int cellWidth) {
        // TODO: implementation
        lineBuffer = new char[cellHeight][lineLength * (cellWidth + spacing)];
    }
    ...
    private void clearLine() {
        // TODO: implementation
        createLineBuffer(lineBuffer[1].length/(font.getWidth()+spacing), font.getHeight(), font.getWidth());
        cursorPosition = 0;
    }
    ...
    public void printCharacter(char character) {
        // TODO: implementation
        if (cursorPosition >= lineBuffer[1].length)
            return;

        char lowerCaseChar = Character.toLowerCase(character);

        char[][] bitmap = font.getBitmap(lowerCaseChar);

        for (int i = 0; i < bitmap.length; i++) {
            for (int j = 0; j < bitmap[i].length; j++) {
                lineBuffer[i][j + cursorPosition] = bitmap[i][j];
            }
            for (int j = 0; j < spacing; j++) {
                lineBuffer[i][j + cursorPosition + bitmap.length - 1] = ' ';
            }
        }

        cursorPosition += font.getWidth() + spacing;
    ...
    public void printString(String string) {
        ...
        printCharacter(string.charAt(0));
